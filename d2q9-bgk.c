...
//                float **sendData, float **recvData, float **sendDataLarge, float **recvDataLarge);
...
// */
// float timestep(const t_param params, t_speed *cells, t_speed *tmp_cells, int *obstacles, const int rank, const int size, const int down_rank, const int up_rank, const int block_size, float *sendData, float *recvData);
...
//   init_tic = tot_tic;
//   initialise(paramfile, obstaclefile, &params, &cells, &tmp_cells, &local_cells, &obstacles, &av_vels, rank, size, &block_size, &sendData, &recvData, &sendDataLarge, &recvDataLarge);

//   /* Init time stops here, compute time starts*/
//   gettimeofday(&timstr, NULL);
//   init_toc = timstr.tv_sec + (timstr.tv_usec / 1000000.0);
//   comp_tic = init_toc;
//   float tot_cells = total_cells(obstacles, params);

//   for (int tt = 0; tt < params.maxIters; tt++)
//   {
//     float tot_u = timestep(params, &local_cells, &tmp_cells, obstacles, rank, size, down_rank, up_rank, block_size, sendData, recvData);
//     if (rank == 0)
//     {
//       MPI_Reduce(MPI_IN_PLACE, &tot_u, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
//       av_vels[tt] = tot_u / total_cells(obstacles, params);
//     }
//     else
//     {
...
//   if(rank != 0)
//   {
//     for (int ii = 0; ii < params.nx*block_size; ii++)
//     {
//       sendDataLarge[ii*9 + 0] = local_cells.speeds0[params.nx + ii]; 
//       sendDataLarge[ii*9 + 1] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 2] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 3] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 4] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 5] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 6] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 7] = local_cells.speeds1[params.nx + ii];
//       sendDataLarge[ii*9 + 8] = local_cells.speeds1[params.nx + ii];
//     }
//     MPI_Send(sendDataLarge, (params.nx*block_size*9), MPI_FLOAT, 0, 0, MPI_COMM_WORLD);

//   }
//   else 
//   {
//     int offset = params.nx*block_size;
//     for (int src = 1; src < size; ++src)
//     {
//       int recvSize;
//       if(params.ny%size == 0) recvSize = (params.nx*block_size*9);
//       else if (src < params.ny%size) recvSize = (params.nx*block_size*9);
//       else recvSize = (params.nx*(block_size - 1) *9);
//       MPI_Recv(recvDataLarge, recvSize, MPI_FLOAT, src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//       for (int ii =0; ii < recvSize/9; ii++)
//       {
//         cells.speeds0[offset + ii] = recvDataLarge[ii*9 + 0];
//         cells.speeds1[offset + ii] = recvDataLarge[ii*9 + 1];
//         cells.speeds2[offset + ii] = recvDataLarge[ii*9 + 2];
//         cells.speeds3[offset + ii] = recvDataLarge[ii*9 + 3];
//         cells.speeds4[offset + ii] = recvDataLarge[ii*9 + 4];
//         cells.speeds5[offset + ii] = recvDataLarge[ii*9 + 5];
//         cells.speeds6[offset + ii] = recvDataLarge[ii*9 + 6];
//         cells.speeds7[offset + ii] = recvDataLarge[ii*9 + 7];
//         cells.speeds8[offset + ii] = recvDataLarge[ii*9 + 8];
//       }
//       offset += recvSize/9;
//     }
//     for (int ii = 0; ii < params.nx * block_size; ii++)
//     {
//       cells.speeds0[ii] = local_cells.speeds0[params.nx + ii];
//       cells.speeds1[ii] = local_cells.speeds1[params.nx + ii];
//       cells.speeds2[ii] = local_cells.speeds2[params.nx + ii];
//       cells.speeds3[ii] = local_cells.speeds3[params.nx + ii];
//       cells.speeds4[ii] = local_cells.speeds4[params.nx + ii];
//       cells.speeds5[ii] = local_cells.speeds5[params.nx + ii];
//       cells.speeds6[ii] = local_cells.speeds6[params.nx + ii];
//       cells.speeds7[ii] = local_cells.speeds7[params.nx + ii];
//       cells.speeds8[ii] = local_cells.speeds8[params.nx + ii];
//     }
//   }
...
//   }
//   finalise(&params, &cells, &tmp_cells, &local_cells, &obstacles, &av_vels, &sendData, &recvData, &sendDataLarge, &recvDataLarge);
//   MPI_Finalize();
//   return EXIT_SUCCESS;
// }
...
// float timestep(const t_param params, t_speed *restrict cells, t_speed *restrict tmp_cells, int *restrict obstacles, const int rank, const int size, const int down_rank, const int up_rank, const int block_size, float* sendData, float* recvData)
// {
...
//   {
//     sendData[ii*9] = cells->speeds0[(params.nx * block_size) + ii];
...
//   MPI_Sendrecv(sendData, (params.nx*9), MPI_FLOAT, up_rank, 0, recvData, (params.nx*9), MPI_FLOAT, down_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  
  
//   for (int ii = 0; ii < params.nx; ii++)
//   {
//     cells->speeds0[ii] = recvData[ii*9];
...
//   MPI_Sendrecv(sendData, (params.nx*9), MPI_FLOAT, down_rank, 1, recvData, (params.nx*9), MPI_FLOAT, up_rank, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
// int initialise(const char *paramfile, const char *obstaclefile,
//                t_param *params, t_speed *cells_ptr, t_speed *tmp_cells_ptr, t_speed *local_cells_ptr,
//                int **obstacles_ptr, float **av_vels_ptr, int rank, int size, int *block_size, float **sendData, 
//                float **recvData, float **sendDataLarge, float **recvDataLarge)
// {
...
//   if (rank = 0)
//     *recvDataLarge = (float *)malloc(sizeof(float) * params->nx * *block_size * 9);
//   else 
//     *sendDataLarge = (float *)malloc(sizeof(float) * params->nx * *block_size * 9);
